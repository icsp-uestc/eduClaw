...
async def generate_profile(student_id: str = "", name: str = "", major: str = "", grade: str = "") -> ToolResponse:
    ...
    # 优先使用传入参数，缺省时 fallback 到当前会话用户
    sid = student_id or student.get("student_id", "demo_user")
    sname = name or student.get("name", "Demo")
    smajor = major or student.get("major", "计算机科学与技术")
    sgrade = grade or student.get("grade", "大二")
    ...
    summary = f"{sname}（{smajor} {sgrade}），整体GPA：{gpa:.2f}，擅长能力：{ABILITY_NAMES[best[0]]}（{best[1].level}级），待提升：{ABILITY_NAMES[worst[0]]}（{worst[1].level}级）。"
